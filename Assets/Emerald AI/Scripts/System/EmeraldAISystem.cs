...
//The code that controls the AI sytem from https://assetstore.unity.com/packages/tools/ai/emerald-ai-3-0-203904#description and has been modified to DevionGames StatHandler Script as well as interact with Unity's ML Package
...
namespace EmeraldAI
{
    ...
    public class EmeraldAISystem : MonoBehaviour
    {
        ...
        void Awake()
        {
            ...
            Attribute stat = statsHandler.GetStat("Health") as Attribute;
            stat.CurrentValue = StartingHealth;
        ...
        private void Start()
        {
            curHealth = statsHandler.GetStatCurrentValue("Health");
            player = GameObject.FindGameObjectWithTag("Player");
            
            //Debug.Log("Health: " + statsHandler.GetStatCurrentValue("Health"));
        ...
        void Update()
        {
            ...
            /*if (Vector3.Distance(player.transform.position, transform.position) <= 10 && !m_NavMeshAgent.isActiveAndEnabled)
            {
                m_NavMeshAgent.enabled = true;
            }
            else if(m_NavMeshAgent.isActiveAndEnabled && Vector3.Distance(player.transform.position, transform.position) > 11)
                m_NavMeshAgent.enabled = false;*/

            //custom death event
            if (statsHandler.GetStatCurrentValue("Health") <= 0 && !IsDead)
            {
                AIBoxCollider.isTrigger = true;
                CurrentHealthFloat = 0;
                EmeraldBehaviorsComponent.DeadState();
            }

            //Increase SO float variable when hit
            if (statsHandler.GetStatCurrentValue("Health") < curHealth)
            {
                elementInteraction.IncreaseplayerHitEnemyVarTotal();
                curHealth = statsHandler.GetStatCurrentValue("Health");
            }
        ...
        public void Wander ()
        {
            ...
            {
                if (m_NavMeshAgent.remainingDistance <= StoppingDistance && !m_NavMeshAgent.pathPending && TimeSinceStart > 1.05f)
                {
        ...
        public void EmeraldAttackEvent()
        {
            if (WeaponTypeRef == WeaponType.Melee)
            {
                ...
                float DistanceCheck = Vector3.Distance(player.transform.position, transform.position);
                if (DistanceCheck <= 1.5f) CustomDamageEvent(CurrentDamageAmount);
            }
            else if (WeaponTypeRef == WeaponType.Ranged)
            {
        ...
        //Implemented to ensure the AI deals damange to player's stat hander component
        public void CustomDamageEvent(float damage)
        {
            GameObject receiver = player;
            StatsHandler receiverHandler = receiver.GetComponent<StatsHandler>();

            //Test player has been found
            if (receiver == null) Debug.LogWarning("Player not found");
            else Debug.LogWarning("Player found");

            //Test player has stats handler
            if (receiverHandler == null) Debug.LogWarning("stat handler not found");
            else Debug.LogWarning("stat handler found");

            //Apply damage to player
            if (receiverHandler.GetStatCurrentValue("Health") > 0)
            {
                receiverHandler.ApplyDamage("Health", damage);
                //add code that interacts with current hit total
                elementInteraction.IncreaseenemtHitPlayerVarTotal();
            }
