...
namespace LevelGenerator.Scripts
{
    ...
    public class LevelGenerator : MonoBehaviour
    {
        ...
        //Search child objects from bottom to top to find last room
        void FindLastRoom() 
        {
            /*if(player == null)
            ...
            for (int i = transform.childCount - 1; i > 0; i--)
            {
                ...
                if (dist > curDist)
                {
                    //Find furthest room from the player
                    if (transform.GetChild(i).name.Contains("Corridor") || transform.GetChild(i).name.Contains("Room"))
                    {
                        //Debug.LogWarning(transform.childCount + ": " + i + ": " + transform.GetChild(i).name);
                        curDist = dist;
                        curRoom = transform.GetChild(i);
                    }
                }
                ...
                {
                    Debug.LogWarning(transform.childCount + ": " + i + ": " + transform.GetChild(i).name);
                    teleporter.transform.position = (transform.GetChild(i).position + (Vector3.forward * 3));
                    return;
                }*/
            }

            teleporter.transform.position = (curRoom.position + (curRoom.forward * 3));
            curRoom = null;
            curDist = 0;
        }
